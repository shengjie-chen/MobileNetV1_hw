...
def uniform_quantize_avgpool(k):
  """均匀量化"""
  class qfn(torch.autograd.Function):
...
class activation_quantize_fn_avgpool(nn.Module):
  def __init__(self, a_bit):
    super(activation_quantize_fn_avgpool, self).__init__()
    assert a_bit <= 8 or a_bit == 32
    self.a_bit = a_bit
    self.uniform_q = uniform_quantize_avgpool(k=a_bit)
