...
def main():
    ...
    model_load_path = './MobileNetV1_Quant_mini.pth'
...
##############
    ...
    ##############
        ...
        for k in list(checkpoint['state_dict']):
            a = k
            if a.startswith('model.5'):
                del checkpoint['state_dict'][a]
            elif a.startswith('model.8'):
                del checkpoint['state_dict'][a]
            elif a.startswith('model.9'):
                del checkpoint['state_dict'][a]
            elif a.startswith('model.10'):
                del checkpoint['state_dict'][a]
            elif a.startswith('model.11'):
                del checkpoint['state_dict'][a]
            elif a.startswith('model.13'):
                del checkpoint['state_dict'][a]
        for k in list(checkpoint['state_dict']):
            a = k
            if a.startswith('model.6'):
                a1=list(a)
                a1[6] = str(5)
                a=''.join(a1)
                checkpoint['state_dict'][a] = checkpoint['state_dict'].pop(k)
            elif a.startswith('model.7'):
                a1=list(a)
                a1[6] = str(6)
                a=''.join(a1)
                checkpoint['state_dict'][a] = checkpoint['state_dict'].pop(k)
            elif a.startswith('model.12'):
                a1=list(a)
                a1[6] = str(7)
                a1.pop(7)
                a=''.join(a1)
    ...
    best_acc = 0.0
    save_path = './MobileNetV1_Quant_mini.pth'
